...
def _seed_if_empty(connection: sqlite3.Connection) -> None:
    if connection.execute("SELECT COUNT(*) AS count FROM tasks").fetchone()["count"] == 0:
        for task in [
    ...
    if connection.execute("SELECT COUNT(*) AS count FROM social_posts").fetchone()["count"] == 0:
        for post in [
    ...
    if connection.execute("SELECT COUNT(*) AS count FROM property_posts").fetchone()["count"] == 0:
        for property_post in [
            PropertyPostCreate(property_name="東新宿リノベーション物件", status="scheduled", due_date="2026-07-01", address="東京都新宿区", memo="写真3枚、駅徒歩情報、周辺施設を入れる"),
...
def list_tasks() -> list[dict[str, Any]]:
    with connect() as connection:
        rows = connection.execute("SELECT * FROM tasks ORDER BY CASE priority WHEN 'urgent' THEN 0 WHEN 'high' THEN 1 WHEN 'medium' THEN 2 ELSE 3 END, COALESCE(due_date, '9999-12-31')").fetchall()
    return [row_to_dict(row) for row in rows]


def insert_task(connection: sqlite3.Connection, task: TaskCreate) -> dict[str, Any]:
    task_id = str(uuid.uuid4())
    connection.execute("""INSERT INTO tasks (id, title, project, status, priority, due_date, channel, progress, source, external_url, notion_page_id, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", (task_id, task.title, task.project, task.status, task.priority, task.due_date, task.channel, task.progress, task.source, task.external_url, task.notion_page_id, now_iso()))
...
def upsert_notion_task(task: TaskCreate) -> str:
    ...
    with connect() as connection:
        ...
        if existing:
            connection.execute("""UPDATE tasks SET title = ?, project = ?, status = ?, priority = ?, due_date = ?, channel = ?, progress = ?, source = 'notion', external_url = ?, updated_at = ? WHERE notion_page_id = ?""", (task.title, task.project, task.status, task.priority, task.due_date, task.channel, task.progress, task.external_url, now_iso(), task.notion_page_id))
            return "updated"
        insert_task(connection, task)
        return "imported"
...
def insert_social_post(connection: sqlite3.Connection, post: SocialPostCreate) -> dict[str, Any]:
    ...
    connection.execute("""INSERT INTO social_posts (id, platform, title, status, scheduled_at, published_at, impressions, likes, comments, shares, clicks, source_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", (post_id, post.platform, post.title, post.status, post.scheduled_at, post.published_at, post.impressions, post.likes, post.comments, post.shares, post.clicks, post.source_url))
...
def insert_property_post(connection: sqlite3.Connection, post: PropertyPostCreate) -> dict[str, Any]:
    post_id = str(uuid.uuid4())
    connection.execute("""INSERT INTO property_posts (id, property_name, platform, status, assignee, due_date, address, memo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)""", (post_id, post.property_name, post.platform, post.status, post.assignee, post.due_date, post.address, post.memo))
...
def dashboard_summary() -> dict[str, Any]:
    ...
    return {"total_tasks": total_tasks, "doing_tasks": doing_tasks, "overdue_tasks": overdue_tasks, "done_tasks": done_tasks, "average_progress": average_progress, "social_posts": len(posts), "total_impressions": total_impressions, "total_engagements": total_engagements, "engagement_rate": engagement_rate, "property_posts_pending": property_posts_pending}

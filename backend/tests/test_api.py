...
def test_create_and_patch_task(tmp_path, monkeypatch):
    ...
    init_db()
    client = TestClient(app)
    created = client.post("/api/tasks", json={"title": "Notion同期テスト", "project": "QA", "status": "todo", "priority": "high", "channel": "Development", "progress": 10})
...
def test_notion_sync_without_secrets_is_safe(tmp_path, monkeypatch):
    ...
    init_db()
    client = TestClient(app)
    payload = client.post("/api/sync/notion").json()
